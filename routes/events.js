...
router.get('/upcoming', async (req, res) => {
    try {
        // Get events with start_date >= current date
        req.query.status = 'published';
        req.query.start_date_from = new Date().toISOString().split('T')[0];
        req.query.sort_by = 'start_date';
        req.query.sort_order = 'asc';
        req.query.limit = req.query.limit || 10;
        
        return eventController.getPublicEvents(req, res);
    } catch (error) {
...
router.post('/:eventId/duplicate', 
    ...
    async (req, res) => {
        try {
            // Implementation would duplicate the event with new dates
            return res.json({
                success: true,
                message: 'Event duplicated successfully',
                data: {
                    original_event_id: req.params.eventId,
                    new_event_id: Math.floor(Math.random() * 10000) // Mock ID
                }
            });
        } catch (error) {
            return res.status(500).json({
 ...
 */
router.patch('/:eventId/registrations/:registrationId/attendance', 
    authenticateToken,
    authorizeRoles(['admin', 'moderator', 'volunteer']),
    param('eventId').isInt({ min: 1 }).withMessage('Invalid event ID'),
    param('registrationId').isInt({ min: 1 }).withMessage('Registration ID must be a positive integer'),
 ...
 * POST /api/events/:eventId/registrations/:registrationId/send-email
 */
router.post('/:eventId/registrations/:registrationId/send-email', 
    authenticateToken,
    authorizeRoles(['admin', 'moderator']),
    param('eventId').isInt({ min: 1 }).withMessage('Invalid event ID'),
    param('registrationId').isInt({ min: 1 }).withMessage('Registration ID must be a positive integer'),
    body('subject').isLength({ min: 1, max: 200 }).withMessage('Subject is required and must not exceed 200 characters'),
    body('message').isLength({ min: 1, max: 2000 }).withMessage('Message is required and must not exceed 2000 characters'),
    body('template').optional().isIn(['reminder', 'confirmation', 'cancellation', 'custom']).withMessage('Invalid template'),
    validateRequest,
    async (req, res) => {
        try {
            // Implementation would send email to specific registration
            return res.json({
                success: true,
